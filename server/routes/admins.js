...
router.post('/', async (req, res) => {
  try {
    ...
    const allowedShift = SHIFTS.includes(req.body?.allowedShift) ? req.body.allowedShift : null;

    let allowedTeamType = null;
    let allowedTeamTypes = [];

    if (role === 'admin') {
      if (!TEAM_TYPES.includes(req.body?.allowedTeamType)) {
        return res.status(400).json({ message: 'Team is required for Admin' });
      }
      allowedTeamType = req.body.allowedTeamType;
    }
    ...
    if (role === 'admin_tl') {
      ...
      if (TEAM_TYPES.includes(req.body?.allowedTeamType)) allowedTeamTypes.push(req.body.allowedTeamType);

      allowedTeamTypes = cleanTeamArray(allowedTeamTypes);

      if (!allowedTeamTypes.length) {
        return res.status(400).json({ message: 'At least 1 team is required for Admin TL' });
      }
    }
    ...
    const doc = await Admin.create({
      ...
      passwordHash,
      role,
      allowedTeamType: role === 'admin' ? allowedTeamType : null,
      allowedTeamTypes: role === 'admin_tl' ? allowedTeamTypes : [],
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (target.role === 'super') return res.status(403).json({ message: 'Cannot edit Super Admin' });
    ...
    if (role === 'admin') {
      ...
      update.allowedTeamType = req.body.allowedTeamType;
...
router.delete('/:id', async (req, res) => {
  const target = await Admin.findById(req.params.id);
  if (!target) return res.status(404).json({ message: 'Not found' });
  if (target.role === 'super') return res.status(403).json({ message: 'Cannot delete Super Admin' });
